...
export class PeopleModel extends Base {
	...
	constructor(init?: Partial<PeopleModel>) {
		...
		if (init) {
			...
			if (typeof init.last_name == "string")
			...
			if (typeof init.phone_home == "string")
				this.phone_home = init.phone_home;
			...
			if (typeof init.is_alive == "boolean")
				this.is_alive = init.is_alive;
			if (
				init.death_dttm instanceof Date ||
				typeof init.death_dttm == "string"
			)
