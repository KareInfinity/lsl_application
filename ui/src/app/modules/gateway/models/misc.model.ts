...
export class ECHierarchyNode {
  ...
  constructor(init?: Partial<ECHierarchyNode>) {
    if (init) {
      ...
      if (typeof init.createdAt == "string" || init.createdAt instanceof Date) {
        this.createdAt = new Date(init.createdAt);
      }
      if (typeof init.updatedAt == "string" || init.updatedAt instanceof Date) {
...
export class NodeWithHierarchy {
  hierarchy_string: string = "";
  node_name: string = "";
  hierarchy: Array<ECHierarchyNode> = [];
  node: ECHierarchyNode = new ECHierarchyNode();
  isas_node: ISASHierarchyNode = new ISASHierarchyNode();
}
