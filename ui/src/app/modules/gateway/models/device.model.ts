...
export class DeviceModel extends Base {
  ...
  constructor(init?: Partial<DeviceModel>) {
    ...
    if (init) {
      ...
      if (init.last_seen instanceof Date || typeof init.last_seen == "string")
      ...
      if (typeof init.ip_address == "string") this.ip_address = init.ip_address;
      ...
      if (init.facility) {
        if (typeof init.facility == "string") {
          try {
            this.facility = new ISASHierarchyNode(JSON.parse(init.facility));
          } catch (error) {
  ...
  constructor(init?: Partial<DeviceModelCriteria>) {
    ...
    if (init) {
      ...
      if (init.device_people) {
        this.device_people = new DevicePeopleModelCriteria(init.device_people);
      }
      if (init.inventory_status) {
        this.inventory_status = new InventoryStatusModel(init.inventory_status);
      }
    }
  }
}
